...
sheet = None  # Глобальная переменная для Google Sheets
GOOGLE_SHEETS_ENABLED = False  # Флаг доступности Google Sheets
...
def init_google_sheets():
    ...
    try:
        ...
        if not google_credentials_json:
            ...
            print("ℹ️ Добавьте JSON ключ в переменные окружения Railway")
        ...
        try:
            ...
            print("ℹ️ Проверьте правильность JSON в переменной окружения")
            GOOGLE_SHEETS_ENABLED = False
            return
        ...
        try:
            ...
            print("⚠️ 1. Создайте таблицу в Google Sheets")
            print("⚠️ 2. Скопируйте ID из URL: https://docs.google.com/spreadsheets/d/ВАШ_ID/")
            print("⚠️ 3. Дайте доступ сервисному аккаунту")
            GOOGLE_SHEETS_ENABLED = False
        ...
        print("⚠️ Добавьте в requirements.txt: gspread google-auth")
        GOOGLE_SHEETS_ENABLED = False
...
init_db()
init_google_sheets()  # Инициализируем Google Sheets при запуске
...
async def update_google_sheets(user_id, username, fandom, subcategory):
    ...
    try:
        ...
        try:
            ...
            print(f"📊 Найден user_id {user_id} в строке {cell.row}")
        except gspread.exceptions.CellNotFound:
            cell = None
            print(f"📝 user_id {user_id} не найден, создаем новую запись")
        
        if cell:  # Пользователь уже есть - обновляем
            row = cell.row
            print(f"🔄 Обновляем строку {row} для user_id {user_id}")
            
            # Обновляем ячейки (колонки с 1)
            sheet.update_cell(row, 1, str(user_id))       # Колонка A - User ID
            sheet.update_cell(row, 2, username)           # Колонка B - Username
            ...
            print(f"➕ Добавляем нового user_id {user_id} в Google Sheets")
            ...
            new_row = [
                ...
                fandom, 
                subcategory,
                current_time,  # First Seen (колонка E)
                current_time,  # Last Updated (колонка F)
                current_time   # Timestamp (колонка G)
            ]
...
async def choose_fandom(callback_query: CallbackQuery, state: FSMContext):
    ...
    await callback_query.message.edit_text(
        ...
        "Чтобы найти подходящего собеседника давай решим что тебе блише?\n"
        "Выбери один вариант:",
        reply_markup=keyboard
    )
